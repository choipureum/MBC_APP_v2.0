  ...
  <div class="wrap-mbic" v-show="initRender">
    ...
    <div v-show="scheduleList.length > 0" class="wrapper thumb_list">
      <ul>
        <li
          class="vod"
          v-on:click="clickInterface('MbicLive', item.ScheduleCode)"
        ...
        >
          <span class="img">
            ...
            <span
              class="vod_btn_play"
              :style="{
                backgroundImage: 'url(' + './static/images/btn_play.png' + ')',
              }"
            ></span>
          </span>
          <div>
  ...
  methods: {
    ...
    InitScheduleData() {
      ...
      $.ajax({
        ...
        cache: true,
        timeout: 10000,
        success: function ({ CH24List }) {
          if (CH24List.length > 0) {
            _this.scheduleList = [...CH24List];
          }
        },
      });
    },
    ...
    clickInterface(...args) {
      var _this = this;
      var link =
        "https://m.imbc.com/onair/nvod/" + parseInt(args[1].substring(1));
      //android
      if (this.userAgent == "android") {
        switch (args[0]) {
          ...
          case "MbicLive":
            if (_this.isNewVersion || _this.isOnAir(args[1])) {
              window.iMBCHandler.setAppOnair(args[1]);
            } else {
              window.iMBCHandler.setAppBanner("WEB", link, "mbicLive");
            }
            break;
        }
      }
    ...
    isV5App() {
      this.isNewVersion = navigator.userAgent.indexOf("MBCTV5") > -1;
    },
  },
};
</script>
