                ...
                <div class="slide">
                    <ul ref="ul">
                        ...
                        <li v-on:click="stateChange(6)">
                            <a style="white-space:nowrap" v-on:click="clickGNBInterface('WEB', 'https://imnews.imbc.com/issue/vote2022/m/main_m.html', '베이징 올릭픽')">베이징 올릭픽</a>
                        </li>
                        -->
                        <li v-for="(item,index) in GnbMenu" v-if="item.state=='Y'" :class="{'on': index == idx}">
                            ...
                            <a v-else v-on:click="clickGNBInterface(item.weblinkpos, item.link, item.menuName)" style="white-space:nowrap">{{item.menuName}}</a>
                        </li>
                    </ul>                
                </div>
            </nav>
