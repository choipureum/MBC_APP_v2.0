  ...
  <div class="chHome nowfree">
    <div v-show="rendering" class="container">
      ...
      <section class="program-wrap">
          ...
          <div id="chHomeWrap" class="chHome-area type-full swiper-container">
            <div :class="{ 'swiper-wrapper': mainTopList.length > 0 }">
              <div
                ...
                :style="{
                  background:
                    'url(' + 'https://img.imbc.com/template/' + item.MImg + ')',
                }"
                v-for="(item, index) in mainTopList"
                v-on:click="clickInterface('topBanner', item.MLinkUrl)"
                v-bind:key="index"
              >
                <a>
                  <div class="img" ref="sliderImg">
                    <img
                      :src="'https://img.imbc.com/template/' + item.MImg"
                      :alt="item.MTitle"
                    />
                  </div>
                  <div class="txt-wrap">
                    <p class="title ellipsis2" v-html="item.MTitle"></p>
                    <p class="program ellipsis" v-html="item.MContent"></p>
                  </div>
                </a>
              </div>
            </div>
        ...
        <!-- 시청중 영상 시작 -->
        <div
          class="onair-wrap nowfree-onair"
          v-if="userSeamList.List.length > 0"
        >
          <h2 class="tit-sec" v-if="userSeamList.username != ''">
            <strong>'{{ userSeamList.username }}'</strong>님이 시청중인 영상
          </h2>
                    ...
                    >
                      ...
                      <div class="bar-area">
                        <span
                          ...
                          :style="{
                            width:
                              (item.MediaTime / item.TotalDuration) * 100 + '%',
                          }"
                          ><strong></strong
                        ></span>
                      </div>
              ...
              <div class="curation">
                <div class="type_row" :id="'proSlide' + index">
                  <div class="slide">
                    <ul>
                      <li
                        v-for="(broadItem, broadIndex) in item.List"
                        v-on:click="
                          clickInterface(
                            'content',
                            broadItem.BroadcastID.toString()
                          )
                        "
                        v-bind:key="broadIndex"
                      >
                        <a
                          ><span
                            class="img"
                            ...
                            ><img
                              :data-src="broadItem.PosterPicture"
                              :alt="broadItem.Title"
                          /></span>
                          <div class="tit" v-html="broadItem.Title"></div
                        ></a>
                      </li>
                    </ul>
                  </div>
          ...
          <div class="curation">
            <div class="type_row" id="bestSlide">
              <div class="slide">
                <ul>
                  <li
                    v-for="(item, index) in best10List"
                    v-on:click="clickInterface('content', item.BroadcastId)"
                    v-bind:key="index"
                  >
                    <a
                      ><span
                      ...
                      /></span>
                      <p class="txt" v-html="item.Title"></p
                    ></a>
                  </li>
                </ul>
              </div>
        ...
        <div class="wrapper wrap-search">
          <div class="tabs">
            <ul>
              <li
                :class="{ on: initLabeling.broad == 'drama' }"
                @click="
                  categoryDataFilter('drama', 1);
                  return false;
                "
              >
                <button>드라마</button>
              ...
              <li
                :class="{ on: initLabeling.broad == 'variety' }"
                @click="
                  categoryDataFilter('variety', 2);
                  return false;
                "
              >
                <button>예능</button>
              ...
              <li
                :class="{ on: initLabeling.broad == 'culture' }"
                @click="
                  categoryDataFilter('culture', 3);
                  return false;
                "
              >
                <button>시사교양</button>
          ...
          <div class="wrap-sorts">
            <form
              ...
              action="return false"
              @submit.prevent="listDataFilter(endYear, initial, keyword)"
            >
              <input
                ref="search"
                type="search"
              ...
              <button type="button" class="btn-search">
                <span
                  ...
                  @click="listDataFilter(endYear, initial, keyword)"
                  :style="{
                    backgroundImage:
                      'url(' + './static/images/ico_search.png' + ')',
                  }"
                  >검색하기</span
                >
              </button>
            ...
            <div class="wrap-select">
              <div class="select-box">
                <select
                  ...
                  v-model="endYear"
                  ref="year"
                  @change="listDataFilter($event.target.value, initial, '')"
                >
                  <option
              ...
              <div class="select-box">
                <select
                  ...
                  v-model="initial"
                  ref="title"
                  @change="listDataFilter(endYear, $event.target.value, '')"
                >
                  <option
                    v-for="(ko, index) in koreans"
                    :value="ko[1]"
                    v-bind:key="index"
                  >
                    {{ ko[0] }}
          ...
          <div
            class="wrap-program"
            :style="freeList.List == 0 ? { height: '150px' } : {}"
          >
            <ul>
              <li
                v-on:click="
                  clickInterface('content', item.BroadcastID.toString())
                "
              ...
              >
                <a>
                  <span
                    ...
                    ><img :data-src="item.PosterPicture" :alt="item.Title"
                  /></span>
          ...
          >
            <div slot="no-results" v-show="freeList.List.length == 0">
              <span
                ...
                "
                >검색하신 프로그램이 존재하지 않습니다. <br />다른 키워드로 다시
                검색해주세요.</span
              >
            </div>
            <div
...
export default {
  ...
  updated() {
    this.$nextTick(function () {
      ...
      if (
        !this.initRender &&
        this.mainTopList.length > 0 &&
        this.best10List.length > 0
      ) {
        sliderBanner();
        ...
        switch (args[0]) {
          ...
          case "userSeam":
            ...
            if (args[1] == "MOVIE") {
              ...
              type = "VOD";
              window.iMBCHandler.setAppContent(type, args[2], "Y");
            }
            break;
