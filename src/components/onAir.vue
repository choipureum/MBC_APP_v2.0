...
<template>
        <div class='wrap_onair' v-show="initRender">
            ...
            <div v-if="programList.length > 0">
                ...
                <div class="wrapper thumb_list">
                    <ul>
                        <li class="vod" v-for="(item,index) in programList" v-on:click="clickInterface('program', item.MobileLiveurl)" :key='index'>
                            <span class="img" v-lazy-container="{ selector: 'img' }"><img :data-src="item.PlayerimageFileurl" :alt="item.ProgramTitle"></span>
                            <div>
                                <span class="title ellipsis2" v-html="item.ProgramTitle"></span> 
                                <span class="program ellipsis">제작발표회</span> 
                                <span class="writer">{{item.PlayerStart.substring(11)}}~{{item.PlayerEnd.substring(11)}}</span>
                            </div>
                        </li>
                    </ul>
                </div>
...
export default {
    ...
    methods: {
        ...
        InitTopBannerData() {
            var _this = this;
            var topBannerUrl = 'https://control.imbc.com/MBCApp/Banner/Live?type='+ checkMobile() +'&category=1&date=' + getDateFormat();
            /**
             * @title 상단 배너 매핑
             */         
            $.ajax({
        ...
        InitSportsData() {
            var _this = this;
            var sportsUrl = 'https://control.imbc.com/Sports/Schedule/V2?date=' + getDateFormat();
            /**
             * @title 스포츠 매핑
             */         
            $.ajax({
        ...
        InitProgramData() {
            var _this = this;
            var scheduleUrl = 'https://control.imbc.com/Program/LiveList?date=' + getDateFormat();
            /**
             * @title 제작발표회 매핑
             */         
            $.ajax({
            ...
            $.ajax({
                ...
                success: function (o) {
                    if (o.length > 0) {
                        var n = o.length - 1;
                        $.each(o, function (index, item) {
                            if(item.Type == "TV" || item.Type == "MBCPLUS") _this.scheduleList.push(item);
                            else if(item.Type == "RADIO") _this.radioList.push(item);
                        });                       
                    }
                }
            });
