...
<template>
  <div class="wrap_home">
        ...
        <ul :class="{ 'swiper-wrapper': topBanner.length > 1 }">
          <li
            ...
            v-on:click="
              clickInterface(
                'banner',
                item.Info.Relation.LinkType,
                item.Info.Relation.LinkURL,
                item.Info.Title,
                item.Info.Relation.ActionType,
                item.Info.Relation.BID,
                item.Info.Relation.NoticeMsg,
                item.Info.Relation.ScheduleCode
              )
            "
            :key="index"
          >
    ...
    <div v-for="(item, index) in itemList" :key="index">
      <!-- 온에어  4.23(진행중)-->
      <div
        v-if="item.Division == 'Live'"
        class="wrapper wrap_player"
        style="position: relative"
        v-on:click="clickInterface('onair', item.DivisionType)"
      >
        <div
          v-show="
            item.Info.Program != '방송 종료' || item.Info.Program != '방송 시작'
          "
        >
          <div class="player onair-view" id="onairwrapper">
            <span class="img" style="display: block"
              ><img
                class="thumb"
                :src="item.Info.Relation.ContentImage"
                :alt="item.Info.Title"
            /></span>
            <span
              class="img player onair-view"
              id="onair"
              style="display: none"
          ...
          <div class="wrap_txt">
            ...
            <div class="tit_info">
              <span class="bg_round">Live</span
              ><span class="s_info">{{ item.Info.BroadDate }}</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div v-else-if="item.Division == 'News'">
        ...
        <div class="wrapper wrap_news">
          ...
          <div class="news_list">
            <div class="tabs">
              <ul>
                <li class="on" v-on:click="selectedCategory = 'A'">
                  <span>MBC</span>
                </li>
            ...
            <ul>
              <li
                v-for="(newsItem, newsIndex) in filteredNews"
                :class="{ first: newsIndex == 0, ellipsis: newsIndex > 0 }"
                :key="newsIndex"
              >
                <a
                  v-on:click="
                    clickInterface(
                      'bannerNews',
                      newsItem.Info.Relation.LinkType,
                      newsItem.Info.Relation.LinkURL,
                      'MBC뉴스'
                    )
                  "
                >
                  <span v-if="newsIndex == 0" class="img">
                    <img
                      :src="newsItem.Info.Relation.ContentImage"
                      :alt="newsItem.Info.Title"
                    />
                  </span>
                  <span
                    :class="{
                      'title ellipsis3': newsIndex === 0,
                      ellipsis: newsIndex > 0,
                    }"
                    ><strong>{{ newsIndex + 1 + "." }}</strong
                    >{{ newsItem.Info.Title }}</span
                  >
                </a>
              </li>
            </ul>
      ...
      <!-- 프로모션 배너 타입 B -->
      <div
        v-else-if="item.Division == 'Banner' && item.DivisionType == 'B'"
        class="wrapper wrap_banner"
        v-on:click="
          clickInterface(
            'banner',
            item.Info.Relation.LinkType,
      ...
      >
        ...
        <a>
          <img
            :data-url="item.Info.Relation.ContentImage"
            :alt="item.Info.Title"
            v-img-lazy-loading
          />
          <div
            class="banner_txt"
            v-bind:class="{
              center: item.Info.Desc == '' && item.Info.Title != '',
            }"
          >
      ...
      <div
        v-else-if="item.Division == 'Promotion'"
        class="wrapper wrap_promo flag1"
        v-on:click="
          clickInterface(
            'banner',
            item.Info.Relation.LinkType,
      ...
      >
        <a>
          ...
          <span
            v-if="
              item.Info.Relation.Keyword == '' ||
              item.Info.Relation.Keyword == '사용안함'
            "
          ></span>
      ...
      <div
        v-else-if="item.Division == 'VOD' && item.DivisionType == 'B'"
        class="wrapper wrap_vod wrap_sports wrap_player"
        :style="
          'background:#27253f url(' +
          item.Info.Image +
          ') no-repeat;background-size: 100%;'
        "
        v-on:click="
          clickInterface('content', item.Division, item.Info.Relation.BID)
        "
      >
        <div
          class="player replayVod"
          :data-bid="item.Info.Relation.BID"
          :data-div="'VODContent' + index"
          :data-title="item.Info.Title"
          style="position: relative"
        >
          <span class="img" style="display: block"
            ><img
              class="thumb img_thumb"
              :data-url="item.Info.Relation.ContentImage"
              :alt="item.Info.Title"
              v-img-lazy-loading
          /></span>
        ...
        <div class="wrap_txt">
          ...
          <a
            class="btn_vod"
            :style="{
              backgroundImage: 'url(' + './static/images/bg-link.png' + ')',
            }"
            >바로가기</a
          >
        </div>
      ...
      <div
        v-else-if="item.Division == 'VOD' && item.DivisionType == 'A'"
        class="wrapper wrap_vod wrap_player"
        :style="
          'background:#27253f url(' +
          item.Info.Image +
          ') no-repeat;background-size: 100%;'
      ...
      >
        <div
          class="vod_gra"
          :style="{
            backgroundImage: 'url(' + './static/images/vod_top_gra.png' + ')',
          }"
        ></div>
        <div
          class="player replayVod"
      ...
      <div
        v-else-if="item.Division == 'Clip' && item.DivisionType == 'A'"
        class="wrapper wrap_player"
        :data-title="item.Info.Title"
        v-on:click="
          clickInterface('content', item.Division, item.Info.Relation.BID)
      ...
      <div
        v-else-if="item.Division == 'Preview'"
        class="wrapper wrap_player"
        :data-title="item.Info.Title"
        v-on:click="
          clickInterface('content', item.Division, item.Info.Relation.BID)
      ...
      <div
        v-else-if="item.Division == 'Special'"
        class="wrapper wrap_player"
        :data-title="item.Info.Title"
        v-on:click="clickInterface('content', 'Sketch', item.Info.Relation.BID)"
      >
        <div
          class="player replayVod"
          :data-bid="item.Info.Relation.BID"
      ...
      <div
        v-else-if="item.Division == 'Clip' && item.DivisionType == 'B'"
        class="wrapper wrap_player"
        :data-title="item.Info.Title"
        v-on:click="
          clickInterface('content', item.Division, item.Info.Relation.BID)
      ...
      <!-- 클래스 flag2 무료 플래그 -->
      <div
        v-else-if="item.Division == 'Banner' && item.DivisionType == 'C'"
        class="wrapper wrap_banners"
        v-on:click="
          clickInterface(
            'banner',
            item.Info.Relation.LinkType,
...
export default {
  ...
  methods: {
    Init() {
      var _that = this;
      var timeline_url =
        "https://control.imbc.com/App/V2/HomeTimeline?type=" +
        this.userAgent +
        "&date=" +
        getDateFormat();
      ...
      if (this.userAgent == "android") {
        switch (args[0]) {
          ...
          case "banner":
            ...
            if (args[4] == "TAB") {
              window.iMBCHandler.goTab(args[1]);
            } else if (args[4] == "FreeVOD") {
              window.iMBCHandler.setAppContent("VOD", args[5], "Y");
            } else if (args[4] == "Clip" || args[4] == "VOD") {
              window.iMBCHandler.setAppContent(args[4], args[5], "N");
            } else if (args[4] == "Live" || args[4] == "MbicLive") {
              window.iMBCHandler.setAppOnair(args[7]);
            } else {
              window.iMBCHandler.setAppBanner(args[1], args[2], args[3]);
            }
            break;
